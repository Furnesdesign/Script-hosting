(function() {
  ...
  function closeDropdown(wrapper) {
    ...
    if (button) {
      // Check if dropdown is open (Webflow adds w--open class)
  ...
  // Also handles loader animation
  let lastCount = 0;
  let stableChecks = 0;
  let totalChecks = 0;
  const minChecksBeforeHide = 10; // Minimum 10 seconds before hiding loader
  const stableChecksRequired = 5; // 5 seconds of stability required
  ...
  setInterval(() => {
    ...
    // Hide loader after minimum time AND count is stable
    if (totalChecks >= minChecksBeforeHide && stableChecks >= stableChecksRequired && loader && loader.style.display !== 'none') {
